...
void main() {
    ...
    vec3 wind = vec3(
        sin(t * 0.07 * uCloudSpeed + uSeed) * 0.5,
        cos(t * 0.05 * uCloudSpeed + uSeed * 1.3) * 0.25,
        0.35
    );
    ...
    for (int i = 0; i < 32; i++) {
        ...
        vec3 plow = p * 0.7 + wind * 0.6 + vec3(0.0, 0.0, uSeed);
        vec3 phigh = p * 1.6 + wind * 1.0 + vec3(0.0, 0.7, uSeed * 1.7);
