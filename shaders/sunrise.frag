...
void main() {
    ...
    // --------------------------------------------------------
    // SUN POSITION — low on horizon, left side
    // --------------------------------------------------------
    vec2 sunPos = vec2(-0.35, 0.05);

    float dist = length(p - sunPos);

    // --------------------------------------------------------
    // SKY COLORS — sunrise palette
    // --------------------------------------------------------
    vec3 horizon = vec3(1.0, 0.55, 0.50);  // warm pink/orange
    vec3 midSky  = vec3(0.95, 0.70, 0.85); // rose
    vec3 zenith  = vec3(0.20, 0.25, 0.45); // deep blue
    ...
    skyColor = mix(skyColor, vec3(1.0, 0.85, 0.85), fog * 0.35);
    ...
    vec3 sunColor = vec3(1.0, 0.75, 0.45); // warm sunrise gold
    ...
    skyColor += vec3(1.0, 0.95, 0.90) * corona * 0.5;
    ...
    vec3 cloudColor = vec3(1.0, 0.75, 0.85); // pink sunrise clouds
